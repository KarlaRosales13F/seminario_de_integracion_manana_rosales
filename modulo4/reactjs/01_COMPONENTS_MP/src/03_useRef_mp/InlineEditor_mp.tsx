...
export default function InlineEditor() {
  ...
  const [saved, setSaved] = useState('Escribe una nota de cliente y guarda')

  function handleSave() {
    const value1 = inputRef.current?.value.trim() ?? ''
    const value2 = textRef.current?.value.trim() ?? ''

    const text1 = value1 === '' ? '(vacio)' : value1
    const text2 = value2 === '' ? '(vacio)' : value2
  ...
  return (
    <div style={{ maxWidth: 340, display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <input
        ref={inputRef}
        defaultValue=""
        placeholder="Nota del cliente sin causar re-renders..."
